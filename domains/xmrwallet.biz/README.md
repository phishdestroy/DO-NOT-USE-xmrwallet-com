# xmrwallet.biz — Escape Domain #2

**xmrwallet.biz** is an alternative domain for the xmrwallet.com scam operation, registered 5 days after xmrwallet.cc during active PhishDestroy investigation.

## Quick Facts
| | |
|---|---|
| **Domain** | xmrwallet.biz |
| **Status** | 🔴 Active |
| **Registered** | **2026-02-09** ← 5 days after .cc |
| **Expires** | 2031-02-09 (5 years prepaid) |
| **Registrar** | Web Commerce Communications Limited dba WebNic.cc |
| **Registrar abuse** | abuse@webnic.cc |
| **IP** | 190.115.31.40 |
| **ASN** | AS59692 — IQWeb FZ-LLC 🇧🇿 Belize City |
| **NS** | ns1/ns2.ddos-guard.net |
| **MX** | mx1/mx2.privateemail.com ← **identical to xmrwallet.com** |
| **SPF** | spf.privateemail.com ← **identical to xmrwallet.com** |
| **TXT** | google-site-verification=OXmFcc_ivkHHczUCE5EFUAlpWiHiNvqjDOl-5u1hNac |
| **WOT** | `8a5554c915e3c17278a7` ← **same token as xmrwallet.cc** |
| **Domain locks** | clientTransferProhibited · clientDeleteProhibited · clientUpdateProhibited · **serverTransferProhibited** |
| **VT domains** | www.xmrwallet.biz · xmrwallet.biz |
| **VT ASN** | AS59692 confirmed |
| **VT files** | 23 file hashes identified |

## Maximum Lock Status
`serverTransferProhibited` is a **registry-level** lock — cannot be removed by the registrar, requires the TLD registry itself (.biz = Neustar) to act. This is the highest possible protection from forced domain transfer or takedown. Not standard for a "volunteer" project.

## WOT Token Match
`wot-verification: 8a5554c915e3c17278a7` — **identical token** across xmrwallet.cc and xmrwallet.biz. Single WOT account controlling reputation of both escape domains simultaneously.

## Same Backend as xmrwallet.com
IP 190.115.31.40 → AS59692 IQWeb FZ-LLC — same ASN as xmrwallet.com (186.2.165.49). Different IP, same operator network.

## VirusTotal File Hashes (23 identified)
```

```
Cross-check: `https://www.virustotal.com/gui/file/[HASH]`

## DNS Map
See [DNSDumpster map](../../docs/img/dnsmap-xmrwallet-biz.png)

## Report
- abuse@webnic.cc
- abuse@ddos-guard.net
- https://www.virustotal.com/gui/domain/xmrwallet.biz
- https://safebrowsing.google.com/safebrowsing/report_phish/
- https://phish.report/

## Full Evidence
https://phishdestroy.github.io/DO-NOT-USE-xmrwallet-com/
