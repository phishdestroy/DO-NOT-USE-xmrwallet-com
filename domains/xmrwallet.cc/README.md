# xmrwallet.cc — Escape Domain #1

**xmrwallet.cc** is an alternative domain for the xmrwallet.com scam operation, registered in February 2026 during active PhishDestroy investigation.

## Quick Facts
| | |
|---|---|
| **Domain** | xmrwallet.cc |
| **Status** | 🔴 Active |
| **Registered** | **2026-02-04** ← same week research published |
| **Expires** | 2034-02-04 (8 years prepaid) |
| **Registrar** | PDR Ltd. d/b/a PublicDomainRegistry.com |
| **Registrar abuse** | abuse@publicdomainregistry.com |
| **IP** | 185.129.100.248 |
| **ASN** | AS57724 — DDOS-GUARD LTD 🇷🇺 Rostov-na-Donu |
| **NS** | ns1/ns2.ddos-guard.net |
| **MX** | mx1/mx2.privateemail.com ← **identical to xmrwallet.com** |
| **SPF** | spf.privateemail.com ← **identical to xmrwallet.com** |
| **TXT** | google-site-verification=o3HCvM6FgHp89HNNASmbe2J3ksVtxMvDzYYk-UP3VsY |
| **WOT** | `8a5554c915e3c17278a7` ← reputation manipulation |
| **VT files** | 23 file hashes identified |
| **VT ASN** | AS57724 confirmed |

## Why This Domain Was Registered
Operator's own words: *"Circumvent Country Blockages: Our official domains: xmrwallet.com, xmrwallet.cc, xmrwallet.biz"*

Registered the same week PhishDestroy published full technical evidence. Paid 8 years in advance. Uses bullet-proof DDoS-Guard infrastructure (Russia). PDR registrar is documented for slow/no abuse response.

## Single Operator Proof
MX records (mx1/mx2.privateemail.com) are **identical across all three domains** — same email inbox = same operator. Not circumstantial — it's technically conclusive.

## WOT Verification
`wot-verification: 8a5554c915e3c17278a7` found in page HTML. The operator registered this domain on Web of Trust to manage trust scores and suppress scam warnings in WOT browser extensions **before the domain was publicly announced**.

## VirusTotal File Hashes (23 identified)
```

```
Cross-check: `https://www.virustotal.com/gui/file/[HASH]`

## DNS Map
See [DNSDumpster map](../../docs/img/dnsmap-xmrwallet-cc.png)

## Report
- abuse@publicdomainregistry.com
- abuse@ddos-guard.net
- https://www.virustotal.com/gui/domain/xmrwallet.cc
- https://safebrowsing.google.com/safebrowsing/report_phish/
- https://phish.report/

## Full Evidence
https://phishdestroy.github.io/DO-NOT-USE-xmrwallet-com/
